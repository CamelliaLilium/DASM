...
def parse_args():
    ...
    parser.add_argument('--tsne_output_dir', type=str,
                        default=os.environ.get('DASM_TSNE_OUTPUT_DIR', os.path.join(PROJECT_ROOT, 'tsne_results')),
                        help='Output directory for t-SNE results')
    parser.add_argument('--tsne_enabled', action='store_true',
                        help='Enable t-SNE visualization')
    parser.add_argument('--tsne_interval', type=int, default=5,
                        help='Execute t-SNE every N epochs (default: 5)')
    parser.add_argument('--tsne_samples_per_class', type=int, default=1000,
                        help='Number of samples per class/domain for t-SNE')
    ...
    parser.add_argument('--tsne_save_data', action='store_true',
                        help='Save features and labels for later re-plotting')
    parser.add_argument('--tsne_fontsize', type=int, default=18,
                        help='Font size for t-SNE plots')
    parser.add_argument('--tsne_batch_size', type=int, default=None,
                        help='Batch size for t-SNE feature extraction (default: batch_size * 2)')
    parser.add_argument('--tsne_num_workers', type=int, default=4,
                        help='Number of workers for t-SNE data loading')
    parser.add_argument('--tsne_n_jobs', type=int, default=-1,
                        help='Number of parallel jobs for t-SNE computation (-1 for all cores)')
    parser.add_argument('--tsne_use_gpu', action='store_true',
                        help='Use GPU-accelerated t-SNE (requires RAPIDS cuML)')
...
def extract_features_for_tsne(model, dataset, device, max_samples_per_class=1000, 
                               batch_size=4096, seed=42, num_workers=4):
    """
    优化版本：提前采样 + GPU批量拼接 + 多进程数据加载
    
    Args:
        model: 模型
        dataset: 数据集（TensorDataset）
        device: 设备
        max_samples_per_class: 每个类别/域的最大采样数
        batch_size: 特征提取的batch_size（可以比训练时更大）
        seed: 随机种子
        num_workers: 数据加载的worker数量
    
    Returns:
        features: (N, d_model) numpy array
        class_labels: (N,) numpy array (Cover=0, Stego=1)
        domain_labels: (N,) numpy array (QIM=0, PMS=1, LSB=2, AHCM=3, Cover=-1)
    """
    ...
    # ========== 步骤1: 提前采样索引（避免提取不需要的特征） ==========
    print(f"  [t-SNE] Pre-sampling indices...")
    all_indices = list(range(len(dataset)))
    
    # 快速获取标签（不需要forward，直接从dataset读取）
    all_labels = []
    all_domain_labels = []
    for idx in all_indices:
        sample = dataset[idx]
        if len(sample) == 3:
            _, label, domain_label = sample
        else:
            _, label = sample
            domain_label = -1
        all_labels.append(label.item() if torch.is_tensor(label) else label)
        all_domain_labels.append(domain_label.item() if torch.is_tensor(domain_label) else domain_label)
    ...
    # 先统计每个Stego域的可用样本数
    stego_domain_counts = {}
    for domain_id in [0, 1, 2, 3]:  # QIM, PMS, LSB, AHCM
        domain_mask = (all_labels == 1) & (all_domain_labels == domain_id)
        domain_indices = np.where(domain_mask)[0]
        stego_domain_counts[domain_id] = min(len(domain_indices), max_samples_per_class)
    
    # Cover 的采样数 = 所有 Stego 域的采样数之和
    total_stego_samples = sum(stego_domain_counts.values())
    
    sampled_indices = []
    cover_indices = np.where(all_labels == 0)[0]
    cover_samples_needed = min(len(cover_indices), total_stego_samples)
    if len(cover_indices) > cover_samples_needed:
        sampled_indices.extend(np.random.choice(cover_indices, cover_samples_needed, replace=False))
    else:
        sampled_indices.extend(cover_indices)
    
    # 采样各 Stego 域
    for domain_id in [0, 1, 2, 3]:  # QIM, PMS, LSB, AHCM
        domain_mask = (all_labels == 1) & (all_domain_labels == domain_id)
        domain_indices = np.where(domain_mask)[0]
        samples_needed = stego_domain_counts[domain_id]
        if len(domain_indices) > samples_needed:
            sampled_indices.extend(np.random.choice(domain_indices, samples_needed, replace=False))
        else:
            sampled_indices.extend(domain_indices)
    
    # 统计采样结果
    cover_count = len([i for i in sampled_indices if all_labels[i] == 0])
    stego_counts = {}
    for domain_id in [0, 1, 2, 3]:
        domain_name = DOMAIN_NAME_MAP.get(domain_id, f'D{domain_id}')
        count = len([i for i in sampled_indices if all_labels[i] == 1 and all_domain_labels[i] == domain_id])
        stego_counts[domain_name] = count
    
    total_stego_count = sum(stego_counts.values())
    print(f"  [t-SNE] Sampled {len(sampled_indices)} samples from {len(all_indices)} total")
    print(f"    Cover: {cover_count}, Stego total: {total_stego_count} (Cover == Stego: {cover_count == total_stego_count})")
    print(f"    Stego breakdown: {', '.join([f'{k}={v}' for k, v in stego_counts.items()])}")
    ...
    with torch.no_grad():
        for batch_data in sampled_loader:
            if len(batch_data) == 3:
                inputs, labels, algorithm_labels = batch_data
                inputs = inputs.to(device, non_blocking=True)
                class_labels = labels.squeeze().long().cpu().numpy()
                domain_labels = algorithm_labels.cpu().numpy()
            else:
                inputs, labels = batch_data
                inputs = inputs.to(device, non_blocking=True)
                class_labels = labels.squeeze().long().cpu().numpy()
                domain_labels = np.full(len(class_labels), -1)
            
            _, features = model(inputs, return_features=True)
            all_features_gpu.append(features)  # 保持在GPU上
            all_class_labels.append(class_labels)
            all_domain_labels.append(domain_labels)
    
    # GPU上拼接，最后一次性转CPU
    features_gpu = torch.cat(all_features_gpu, dim=0)
...
def save_tsne_data(features, class_labels, domain_labels, epoch, output_dir):
    """
    保存t-SNE相关数据以便后续重新绘图
    ...
    data_file = os.path.join(output_dir, f'tsne_data_epoch_{epoch}.npz')
    np.savez_compressed(
        data_file,
        features=features,
    ...
    print(f"  Saved t-SNE data: {data_file}")


def plot_tsne_visualization(features, class_labels, domain_labels, epoch, output_dir,
                            perplexity=30, n_iter=1000, random_state=42, fontsize=18,
    ...
    Args:
        ...
        random_state: 随机种子
        fontsize: 字体大小
        n_jobs: t-SNE并行线程数（-1表示使用所有CPU核心）
        use_gpu: 是否使用GPU加速（需要RAPIDS cuML）
    """
    os.makedirs(output_dir, exist_ok=True)
    
    # 执行t-SNE降维到3D
    print(f"  Computing t-SNE (epoch {epoch}, n_jobs={n_jobs})...")
    
    if use_gpu:
        try:
            # 尝试使用RAPIDS cuML的GPU加速t-SNE
            from cuml.manifold import TSNE as cuTSNE
            tsne = cuTSNE(n_components=3, perplexity=perplexity, n_iter=n_iter,
                         random_state=random_state)
            features_3d = tsne.fit_transform(features)
            print(f"  Using GPU-accelerated t-SNE (RAPIDS cuML)")
        except ImportError:
            print(f"  Warning: RAPIDS cuML not available, falling back to CPU")
            use_gpu = False
    ...
    if cover_mask.sum() > 0:
        ax1.scatter(features_3d[cover_mask, 0], features_3d[cover_mask, 1], features_3d[cover_mask, 2],
                   c='orange', label='Cover', s=30, alpha=0.7, edgecolors='darkorange', linewidths=0.5)
    if stego_mask.sum() > 0:
        ax1.scatter(features_3d[stego_mask, 0], features_3d[stego_mask, 1], features_3d[stego_mask, 2],
                   c='blue', label='Stego', s=30, alpha=0.7, edgecolors='darkblue', linewidths=0.5)
    
    ax1.set_xlabel('Dimension 1', fontsize=fontsize, labelpad=10)
    ax1.set_ylabel('Dimension 2', fontsize=fontsize, labelpad=10)
    ax1.set_zlabel('Dimension 3', fontsize=fontsize, labelpad=10)
    ax1.set_title('Cover vs All Stego', fontsize=fontsize+2, fontweight='bold', pad=15)
    ax1.legend(fontsize=fontsize-2, loc='upper right')
    ax1.tick_params(labelsize=fontsize-2)
    ax1.grid(True, alpha=0.3)
    
    # 子图2-5: Cover vs 各个隐写域
    domain_names = ['AHCM', 'LSB', 'PMS', 'QIM']
    domain_ids_map = {'AHCM': 3, 'LSB': 2, 'PMS': 1, 'QIM': 0}
    
    for idx, domain_name in enumerate(domain_names):
        ax = fig.add_subplot(1, 5, idx + 2, projection='3d')
        ...
        if domain_stego_mask.sum() > 0:
            ax.scatter(features_3d[domain_stego_mask, 0], features_3d[domain_stego_mask, 1],
                      ...
                      c='blue', label=domain_name, s=30, alpha=0.7, edgecolors='darkblue', linewidths=0.5)
        
        ax.set_xlabel('Dimension 1', fontsize=fontsize, labelpad=10)
        ax.set_ylabel('Dimension 2', fontsize=fontsize, labelpad=10)
        ax.set_zlabel('Dimension 3', fontsize=fontsize, labelpad=10)
        ax.set_title(f'Cover vs {domain_name}', fontsize=fontsize+2, fontweight='bold', pad=15)
        ax.legend(fontsize=fontsize-2, loc='upper right')
        ax.tick_params(labelsize=fontsize-2)
        ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    save_path = os.path.join(output_dir, f'tsne_{epoch}.png')
    plt.savefig(save_path, dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
    print(f"  Saved t-SNE plot: {save_path}")
    
    return features_3d  # 返回3D坐标以便后续使用
...
def train_model(model, train_loader, test_loader, optimizer, criterion, scheduler, args):
    ...
    for epoch in range(args.epochs):
        ...
        # ========== t-SNE Visualization ==========
        if args.tsne_enabled and (epoch + 1) % args.tsne_interval == 0:
            try:
                print(f"  [t-SNE] Extracting features for epoch {epoch+1}...")
                
                # 获取原始dataset（需要从test_loader.dataset获取）
                test_dataset = test_loader.dataset
                
                # 确定batch_size（优先使用tsne_batch_size，否则使用训练batch_size的2倍）
                tsne_batch_size = args.tsne_batch_size if args.tsne_batch_size is not None else args.batch_size * 2
                
                features, class_labels, domain_labels = extract_features_for_tsne(
                    model, test_dataset, device,
                    max_samples_per_class=args.tsne_samples_per_class,
                    batch_size=tsne_batch_size,
                    seed=args.seed,
                    num_workers=args.tsne_num_workers
                )
                
                # 保存数据（如果启用）
                if args.tsne_save_data:
                    save_tsne_data(features, class_labels, domain_labels, epoch+1, args.tsne_output_dir)
                
                # 绘制t-SNE图（使用并行化）
                features_3d = plot_tsne_visualization(
                    features, class_labels, domain_labels, epoch+1, args.tsne_output_dir,
                    perplexity=args.tsne_perplexity,
                    n_iter=args.tsne_n_iter,
                    random_state=args.seed,
                    fontsize=args.tsne_fontsize,
                    n_jobs=args.tsne_n_jobs,
                    use_gpu=args.tsne_use_gpu
                )
                
                # 保存3D坐标（可选）
                if args.tsne_save_data:
                    coord_file = os.path.join(args.tsne_output_dir, f'tsne_coords_epoch_{epoch+1}.npz')
                    np.savez_compressed(coord_file, coords_3d=features_3d)
