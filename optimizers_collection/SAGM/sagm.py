...
class SAGM(torch.optim.Optimizer):
    """
    Sharpness-Aware Gradient Matching (SAGM) Optimizer

    基于 /root/autodl-tmp/baseline_optimizers/SAGM/domainbed/sagm/sagm.py 提取。
    """
        ...
        if not by:
            ...
            norm = torch.norm(
                torch.stack(
                    [
                        (
                            (torch.abs(p.data) if weight_adaptive else 1.0) * self.state[p][by]
                        ).norm(p=2)
                        for group in self.param_groups
                        for p in group["params"]
                        if p.grad is not None
