...
from models_collection.SS_QCCN.trainer import train_and_test_ss_qccn
...
def _write_temp_split(base_dir: str, x: np.ndarray, y: np.ndarray) -> Tuple[str, str]:
    ...
    for i, (sample, label_info) in enumerate(tqdm(zip(x, y), total=len(x), desc='Writing temp files (SS-QCCN)')):
...
def run_ss_qccn_domain_generalization(args) -> None:
    """Run SS-QCCN with source-domain training and target-domain testing.
    ...
    - Writes filtered splits to SS-QCCN local temp directories, then trains/tests
    - Saves model artifacts under SS-QCCN local models directory, isolated by domain tuple
    ...
    # SS-QCCN-local directories
    root = os.environ.get(
        'DASM_SS_QCCN_ROOT',
        os.path.join(PROJECT_ROOT, 'models_collection', 'SS_QCCN'),
    )
    models_dir = os.path.join(root, 'models', run_tag)
    temp_train_dir = os.path.join(root, 'temp', run_tag)
    temp_test_dir = os.path.join(root, 'temp_test', run_tag)

    os.makedirs(models_dir, exist_ok=True)
    os.makedirs(os.path.dirname(temp_train_dir), exist_ok=True)
    os.makedirs(os.path.dirname(temp_test_dir), exist_ok=True)

    # Materialize filtered splits as SS-QCCN txt format
    ...
    # Train and test SS-QCCN
    print('Running SS-QCCN with domain generalization (source -> target)...')
    train_and_test_ss_qccn(train_steg, train_cover, test_steg, test_cover, models_dir)

    # Always test on individual domains and save results
    print('Testing SS-QCCN on individual domains...')
    ...
    from testing_utils import test_ss_qccn_model
    test_ss_qccn_model(models_dir, args)
